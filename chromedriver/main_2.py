...
password = (''.join(random.choice(string.ascii_letters) for l in range(15)))
...
try:
    ...
    if result_response:
        ...
        payload = {"fingerprint": "","email": email,"username":username,"password":password,"invite":'null',"consent":'true',"date_of_birth":f"{birthday_year}-{birthday_month}-{birthday_day}","gift_code_sku_id":'null',"captcha_key":captcha_code}
    ...
    # if result:
    #     code = result['code']
    #     driver.execute_script(f'document.getElementsByName("g-recaptcha-response")[0].innerText="{code}";')
    #     driver.execute_script(f'document.getElementsByName("h-captcha-response")[0].innerText="{code}";')
    # iframe = driver.find_element(By.XPATH, '//*[@id="app-mount"]/div[2]/div/div[1]/div/div/div/section/div/div[2]/div/iframe')
